...
resource "aws_eip" "eip" {
  instance = aws_instance.QuantaraAI-instance.id
  domain   = "vpc"
}
...
resource "aws_volume_attachment" "example_attachment" {
  ...
  volume_id   = aws_ebs_volume.QuantaraAI-volume.id
  instance_id = aws_instance.QuantaraAI-instance.id

  # Forces detachment of the volume before destroying, allowing Terraform to manage the volume's lifecycle
  force_detach = true
}

resource "aws_instance" "QuantaraAI-instance" {
